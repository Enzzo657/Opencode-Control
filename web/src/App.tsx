  ...
  async function submit(prompt: string, attachments: Attachment[]) {
    ...
    try {
      ...
      if (invocation) {
        ...
        await api(`/api/v1/projects/${project.id}/sessions/${encodeURIComponent(session.id)}/commands/${encodeURIComponent(invocation.name)}`, { method: "POST", ...jsonBody({ arguments: invocation.arguments, ...(variant ? { variant } : {}) }) });
      } else {
        const mentions = mentionedAgents(prompt, agents);
