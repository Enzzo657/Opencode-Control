...
function readSessionSelection(projectId: string, sessionId: string): { agent: string; model: string } | null { try { const value = JSON.parse(window.localStorage.getItem(`control-session-selection:${projectId}:${sessionId}`) ?? "null") as unknown; return value && typeof value === "object" && typeof (value as { agent?: unknown }).agent === "string" && typeof (value as { model?: unknown }).model === "string" ? value as { agent: string; model: string } : null; } catch { return null; } }
function rememberSessionSelection(projectId: string, sessionId: string, agent: string, model: string) { window.localStorage.setItem(`control-session-selection:${projectId}:${sessionId}`, JSON.stringify({ agent, model })); }
