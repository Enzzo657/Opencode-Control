...
сегодня, 7 дней, 30 дней или всё доступное время. Usage считается по assistant messages
...
динамику по дням и breakdown по моделям, providers, agents и проектам. В global scope
недоступный проект не ломает всю сводку: интерфейс показывает частичный результат и
явное предупреждение. Аналитика строится по истории, которая ещё доступна в OpenCode;
удалённые Sessions в totals не сохраняются.
