...
Studio показывает три связанных, но разных состояния MCP:
...
Если общий MCP включен, а пользователь выключает его только для проекта, Studio
...
При повторном включении Studio не оставляет лишнее `{ "enabled": true }`, если
...
явно удаляет project override. Если в локальной записи есть другие поля, Studio
удаляет только избыточный `enabled`, сохраняя остальные проектные настройки.

Изменения записываются сразу. Уже работающий managed OpenCode server Studio
...
Если пользователь сам подключил отдельно запущенный external endpoint, Studio
